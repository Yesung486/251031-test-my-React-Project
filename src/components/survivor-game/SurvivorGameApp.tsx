...
import { GameState, InventoryItem, EquippedState, Rarity, EquipmentType, Skin, UserProfile, Chapter } from './types';
...
import { 
  ...
  Key, Clock, Scroll, FileText, Sparkles, X, ArrowRight, CheckCircle, Crown, Lock, LogIn, LogOut, Heart, ArrowLeft, Gift, Star, Hand, BookOpen, Calculator,
  Flame, Snowflake, Ghost, Crosshair, Aperture, Biohazard, Cpu
} from 'lucide-react';
...
const CharacterPreview = ({ skinId }: { skinId: string }) => {
    ...
    const renderSkinVisual = () => {
        ...
        if (type === 'LIGHTNING') {
            return (
                <div className="relative flex items-center justify-center">
                    <div className="absolute inset-0 bg-yellow-400 blur-[50px] opacity-30"></div>
                    <div className="relative z-10 w-36 h-36 flex items-center justify-center">
                        <Zap className="w-full h-full text-yellow-300 fill-yellow-100 drop-shadow-[0_0_25px_#fef08a] animate-[pulse_0.2s_infinite]" strokeWidth={1} />
                    </div>
                    <div className="absolute inset-[-10px] border-2 border-yellow-300 rounded-full border-dashed animate-[spin_5s_linear_infinite]"></div>
                </div>
            );
        }
    ...
    return (
        <div className="relative w-48 h-48 sm:w-64 sm:h-64 flex items-center justify-center transition-all duration-300">
            ...
            {/* Name Tag - Only shows on hover or always if desired, kept absolute to not mess flow */}
